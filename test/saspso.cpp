...
int main() {
	...
	std::unique_ptr<Optimizer<2>> opt = std::make_unique<SASPSO<2>>(problem, 200, 2000, 1e-10);
	opt->initialize();
	opt->optimize();
	opt->print_results();
	...
	std::unique_ptr<Optimizer<2>> opt_p = std::make_unique<SASPSO<2>>(problem, 300, 2000, 1e-10);
