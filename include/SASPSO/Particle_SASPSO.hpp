...
public:
	/**
	 * @brief Construct a new Particle_SASPSO object
	 *
	 * @param problem shared pointer to the problem to be optimized
	 */
	Particle_SASPSO(const std::shared_ptr<Problem<dim>> &problem,
	 ...
	 */
	bool is_better_than(const Particle_SASPSO<dim> &other, double violation_threshold, double tol) const;
...
#include "SASPSO/Particle_SASPSO.cpp"